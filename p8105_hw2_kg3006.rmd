...
```{r, echo=FALSE}
#filtering for January 2020 and January 2021
zillow_data %>%
  filter(month(date) == 1 & year(date) %in% c(2020, 2021)) %>%
  group_by(zip_code, year = year(date), month = month(date)) %>%
  ...
  mutate(price_drop = zori_2020 - zori_2021) %>% 
  left_join(zillow_zip %>% select(zip_code, county, neighborhood), by = "zip_code") %>%
  arrange(desc(price_drop)) %>%
  select(zip_code, price_drop, county, neighborhood) %>%
...
The largest drops in prices all came from NY county, where neighborhoods such as Lower Manhattan, the Upper West Side, the Lower East side had the largest price drops. 
